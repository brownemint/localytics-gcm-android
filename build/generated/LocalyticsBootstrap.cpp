...
#define TAG "pw.localytics"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Localytics_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Localytics.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::LocalyticsBindings::lookupGeneratedInit(
...
static void Localytics_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Localytics_getBinding)->GetFunction());
}

static void Localytics_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::LocalyticsBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry LocalyticsBinding = {
	"pw.localytics",
	Localytics_init,
	Localytics_dispose
};
...
Java_pw_localytics_LocalyticsBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("pw.localytics", &LocalyticsBinding);
	titanium::KrollBindings::addExternalLookup(&(::LocalyticsBindings::lookupGeneratedInit));
}
